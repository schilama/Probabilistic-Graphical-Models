...
class CPT(object):
    ...
    def __init__(self,node,listofparents):
        ...
        #list of categorical values for current node
...
class BayesNetCustom(object):
    ...
    def __init__(self):
        ...
        self.Acpt = CPT('A',[])
        self.Gcpt = CPT('G',[])
        self.CPcpt = CPT('CP',['HD','EIA'])
        self.BPcpt = CPT('BP',['A','G'])
        self.CHcpt = CPT('CH',['A','G'])
        self.ECGcpt = CPT('ECG',['HD','EIA','HR'])
        self.HRcpt = CPT('HR',['BP','HD'])
        self.EIAcpt = CPT('EIA',['HD','BP','HR'])
        self.HDcpt = CPT('HD',['CH','BP','A','G'])
        #pass
    ...
    def set(self, target_variable, condition_variables, value):
        '''
        This method sets a parameter value in your BayesNet to the given value
    ...
    def genericfit(self,data,currentcpt):
        ...
        if currentcpt.listofparents == []:
            ...
            for p in currentcpt.permutation:
                ...
                parametermle = (numofp+1)/(denom+(1*len(currentcpt.nodecategories)))
